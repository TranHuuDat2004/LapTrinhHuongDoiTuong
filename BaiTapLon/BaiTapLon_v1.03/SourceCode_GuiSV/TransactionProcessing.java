        ...
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                if (accountType.equals("BA")) {
                    // Tao ArrayList tam thoi de so sanh gia tri
                    ArrayList<BankAccount> tempPayments = new ArrayList<>(); 
                    for (Payment payment : paymentObjects) {
                        if (payment instanceof BankAccount) {
                            BankAccount bankAccount = (BankAccount) payment;
                            // Neu tai khoan nay co so tien giao dich thanh cong lon hon max, cap nhat max va ArrayList
                            if (bankAccount.getAccountNumber() == accountInfo && totalAmount >= max) {
                                if (totalAmount > max) {
                                    max = totalAmount;
                                    tempPayments.clear();
                                }
                                tempPayments.add(bankAccount);
                            }
                        }
                    }
    
                    // Lay ArrayList tam thoi vao ArrayList hien tai
                    largestPayments.addAll(tempPayments);
                }
            }
    ...
    public void processTransactionWithDiscount(String path) {
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                double totalAmount = Double.parseDouble(components[1].trim());
                String purpose = components[2].trim();                
                int accountId = Integer.parseInt(components[4].trim());    
                for(Payment payment: paymentObjects){
                    if (payment != null) {
                        if (purpose.equals("Clothing") && totalAmount > 500) {
                            // Kiem tra khach hang nhan duoc uu dai
                            if (payment instanceof EWallet) {
                                EWallet ewallet = (EWallet) payment;
                                IDCard idCard = ewallet.getIdCard();
                                if (idCard != null && ((idCard.getGender().equals("Female") && idCard.getAge() < 18) ||
                                        (idCard.getGender().equals("Male") && idCard.getAge() < 20))) {
                                    // Giam gia
                        ...
                        boolean paymentStatus = payment.pay(totalAmount);
                        if (paymentStatus) {
                            System.out.println("Payment successful for bill ID " + billId);
                        } else {
                            System.out.println("Payment unsuccessful for bill ID " + billId);
                        }
                    } else {
                        System.out.println("Payment object not found for account ID " + accountId);
                    }
                }
